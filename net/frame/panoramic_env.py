...
class panoramic_env:
    def __init__(self, all_cooked_time, all_cooked_bw, random_seed=RANDOM_SEED):
        ...
        # 视频frame大小
        # frame_id:[0,TOTAL_VIDEO_CHUNCK)/fps:(?)/tile:(1*1,2*3,4*6,8*12,16*24)/tile_id:[0,tile_cnt)/tile_size:(?)/tile_bitrate:[0,BITRATE_LEVELS)
        self.video_size = []
        ...
        # frame目标检测后回传信息
        # frame_id:[0,TOTAL_VIDEO_CHUNCK)/fps:(?)/tile:(1*1,2*3,4*6,8*12,16*24)/tile_setings:[0, (len(tile_size_set) * BITRATE_LEVELS) ^ tile_cnt)
    ...
    def get_video_diff(self, video_setings):
        ...
        return self.video_diff_s[self.video_frame_counter][video_setings['fps']][video_setings['tile']][tile_seting]
    ...
    def get_video_info(self,video_setings):
        ...
        tile_bitrate = video_setings['tile_bitrate']
        tile_seting = self.tile_seting_to_int(zip(tile_size, tile_bitrate))
        # print(self.video_frame_counter, video_setings['fps'], video_setings['tile'], tile_seting)
        return self.video_info_s[self.video_frame_counter][video_setings['fps']][video_setings['tile']][tile_seting], self.get_video_diff(video_setings)
    

    def get_video_frame(self, video_setings):

        video_frame_size_s = self.video_size[self.video_frame_counter][video_setings['fps']][video_setings['tile']]
        tile_cnt = video_setings['tile'][0]*video_setings['tile'][1]
        tile_size = video_setings['tile_size']
        tile_bitrate = video_setings['tile_bitrate']

        # frame level
        # frame_finish_time = video_frame_counter + 1
        if self.last_mahimahi_time < self.video_frame_counter + 1:
            self.last_mahimahi_time = self.video_frame_counter + 1
        start_time = self.last_mahimahi_time
        delay = 0.0
        frame_size = 0.0
        # 串行传输
        for tile_id in range(tile_cnt):
            video_frame_size = video_frame_size_s[tile_id][tile_size[tile_id]][tile_bitrate[tile_id]]
            video_frame_counter_sent = 0
            ...
            while True:
                ...
                if video_frame_counter_sent + packet_payload > video_frame_size:
                    fractional_time = (video_frame_size - video_frame_counter_sent) / throughput / PACKET_PAYLOAD_PORTION
                    delay += fractional_time
                    self.last_mahimahi_time += fractional_time
                    break

                video_frame_counter_sent += packet_payload
                delay += duration
        ...
        delay += LINK_RTT
        # video_frame_remain:有多少frame停留在发送队列
        video_frame_remain = self.last_mahimahi_time - self.video_frame_counter - 1
        latency = video_frame_remain
        video_frame_remain = math.floor(video_frame_remain)
        self.video_frame_counter += 1

        net_info = (start_time, self.last_mahimahi_time, delay, frame_size, video_frame_remain, latency)
        video_info = self.get_video_info(video_setings)
        end_of_video = False
        if self.video_frame_counter >= TOTAL_VIDEO_CHUNCK-1:
            end_of_video = True
            self.video_frame_counter = 0
            self.trace_idx += 1
